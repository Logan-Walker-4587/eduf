...
def init_session_db():
    ...
    c.execute(
        '''CREATE TABLE IF NOT EXISTS sessions (
                ...
                flashcards TEXT,
                created_at TEXT,
                shared TEXT DEFAULT 'False'
         )'''
    )
    # Check and add shared column if missing
    c.execute("PRAGMA table_info(sessions)")
    columns = [col[1] for col in c.fetchall()]
    if "shared" not in columns:
        c.execute("ALTER TABLE sessions ADD COLUMN shared TEXT DEFAULT 'False'")
    conn.commit()
    conn.close()
...
# ---------------------------------------------------------------------------------
def create_session(session_name, flashcards, shared=False):
    conn = sqlite3.connect("users.db")
    c = conn.cursor()
    created_at = time.strftime("%Y-%m-%d %H:%M:%S")
    c.execute("INSERT INTO sessions (username, session_name, flashcards, created_at, shared) VALUES (?, ?, ?, ?, ?)",
              (st.session_state["user"], session_name, json.dumps(flashcards), created_at, str(shared)))
    conn.commit()
    conn.close()
...
def get_community_sessions(current_user):
    conn = sqlite3.connect("users.db")
    c = conn.cursor()
    c.execute("SELECT id, username, session_name, flashcards, created_at FROM sessions WHERE username != ? AND shared = 'True'", (current_user,))
    sessions = c.fetchall()
    conn.close()
    return sessions
...
def signup(username, password):
    ...
    password_rules = [
        ...
        ("At least one number", lambda p: any(c.isdigit() for c in p)),
        ("At least one special character", lambda p: any(c in "!@#$%^&*()-_=+[]{}|;:'\",.<>?/`~" for c in p))
    ]
    
    violations = [rule for rule, check in password_rules if not check(password)]
    
    if violations:
        st.error("Password must meet the following criteria:")
        for violation in violations:
            st.markdown(f"<span style='color:red;'>{violation}</span>", unsafe_allow_html=True)
...
def main():
    ...
    # ---------------------- SAVED FLASHCARDS SECTION -------------------------------
        ...
        # Check if the share flag exists in session state, if not initialize it
        if "share_to_community" not in st.session_state:
            st.session_state["share_to_community"] = False
        ...
        if sessions:
            ...
            if st.session_state["share_to_community"]:
                if st.button("Don't Share"):
                    st.session_state["share_to_community"] = False
                    st.success("Flashcards will not be shared with the community.")
            else:
                if st.button("Share to Community"):
                    st.session_state["share_to_community"] = True
                    st.success("Flashcards will be shared with the community.")
                    conn = sqlite3.connect("users.db")
                    c = conn.cursor()
                    c.execute("UPDATE sessions SET shared = 'True' WHERE username = ? AND session_name = ?", (st.session_state["user"], session_name))
                    conn.commit()
                    conn.close()
        else:
