...
# ------------------------- CONFIGURATION -----------------------------------------
# ---------------------------------------------------------------------------------
COHERE_API_KEY = "u9Yy7eYKjsb7AV6SKLxGsnPepaHLD6LOW5B43U9p"
...
# ------------------------- COHERE AI FUNCTIONS -----------------------------------
# ---------------------------------------------------------------------------------
def generate_flashcard_response_cohere(pdf_text, user_input):
    ...
    try:
        ...
        for event in response:
            if event.type == "content-delta":
                flashcard_text += event.delta.message.content.text
...
def generate_test_questions_cohere(pdf_text):
    max_text_length = 1500
    pdf_text = pdf_text[:max_text_length]
    prompt_template = (
        "Generate 10 multiple-choice questions from this document:\n\n'{pdf_text}'\n\n"
        "For each question, provide a JSON object with keys: 'question', 'options' (a list of 4), and 'correct'. "
...
def generate_test_insights_cohere(score):
...
def main():
    ...
    # ---------------------- FLASHCARDS SECTION -----------------------------------
        ...
        if uploaded_file:
            ...
            # Generate initial flashcard only if one isn't already present
            if "current_flashcard" not in st.session_state:
                user_input = st.text_input("Enter topic/question for flashcard:", key="flashcard_query")
                if user_input:
                    flashcard = generate_flashcard_response_cohere(pdf_text, user_input)
                ...
                with st.container():
                    ...
                    with col1:
                        if st.button("Simplify Explanation"):
                            simplified = generate_flashcard_response_cohere(pdf_text, "Simplify this explanation")
                    ...
                    with col2:
                        if st.button("Generate New"):
                            if "flashcard_query" in st.session_state and st.session_state["flashcard_query"]:
                                new_flashcard = generate_flashcard_response_cohere(pdf_text, st.session_state["flashcard_query"])
        ...
        if "pdf_text" not in st.session_state:
            ...
            if "test_questions" not in st.session_state:
                if st.button("Generate Test Questions"):
                    test_questions = generate_test_questions_cohere(st.session_state["pdf_text"])
                    if test_questions:
            ...
            if "test_questions" in st.session_state:
                ...
                if st.button("Submit Test"):
                    ...
                    insights = generate_test_insights_cohere(score)
